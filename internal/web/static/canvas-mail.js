...
export function normalizeMailHeadersContext(event) {
...
async function fetchMailCapabilities(eventId, context) {
  ...
  try {
    ...
    if (requestSeq !== mailCapabilitiesRequestSeq || getActiveTextEventId() !== eventId) return;
...
function stopMailRecordingMedia(recording) {
  if (recording?.mediaRecorder) {
    try {
      ...
      // recorder might already be stopping/stopped
    }
  }
...
async function dispatchMailAssistAction(eventId, context, invocation) {
  ...
  try {
    ...
    if (getActiveTextEventId() !== eventId) return;
...
export function flushPendingUndoAction() {
...
export function renderMailArtifact(eventId, context) {
  const e = getEls();
  const state = getMailViewState(context);
...
export function clearMailInteractionHandlers() {
  const e = getEls();
  flushPendingUndoAction();
...
export function setActiveMailContext(context) {
  activeMailContext = context;
}
